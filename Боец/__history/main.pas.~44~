...
if Panel8.Caption='Определение победителя' then
  begin
    Panel8.Caption:='';
  end
  Else
//if Panel8.Caption='' then
  begin
    Panel8.Caption:='Определение победителя';
    Label14.Caption:='0';
    Label16.Caption:='0';
    Label18.Caption:='0';
    Label15.Caption:='0';
...
begin
  ...
  Label5.Caption:='Оборудование подключено';
...
begin
  ...
  Label5.Caption:='Оборудование отключено';
...
// проверка выхода из паузы во время боя
//or (stringgrid1.Cells[3,3]='идет...') or (stringgrid1.Cells[3,5]='идет...') then

if (stringgrid1.Cells[3,1]='идет...') then
  begin
    panel8.Caption:='Раунд 1';
  end;

if (stringgrid1.Cells[3,3]='идет...') then
  begin
    panel8.Caption:='Раунд 2';
  end;

if (stringgrid1.Cells[3,5]='идет...') then
  begin
    panel8.Caption:='Раунд 3';
  end;
...
begin
// Считаем кнопки если идет первый второй или третий раунд


 // begin
  ComPort1.ReadStr(Str, Count);
  memo1.Lines.Add(Str);   // вывод в лог
  str:=only_numbers(str);
  Edit2.Text:=Str;
if Panel8.Caption<>'' then
begin
...
label95.Caption:='';
 ...
 // button3.Enabled:=true;
  ...
  stringgrid1.Cells[1,0]:='мин';
  stringgrid1.Cells[2,0]:='сек';
  stringgrid1.Cells[3,0]:='статус';

  stringgrid1.Cells[0,1]:='Раунд 1';
  stringgrid1.Cells[0,2]:='Перерыв 1';
  stringgrid1.Cells[0,3]:='Раунд 2';
  stringgrid1.Cells[0,4]:='Перерыв 2';
  stringgrid1.Cells[0,5]:='Раунд 3';
...
begin
  ...
  //label33.Caption:='2';
  //label34.Caption:='0';
  //устанавливаем время раундов в сценарии
...
// начало сценария старт первого раунда
  ...
  if (StringGrid1.Cells[3,1]='') and (StringGrid1.Cells[3,2]='') and (StringGrid1.Cells[3,3]='') and (StringGrid1.Cells[3,4]='') and (StringGrid1.Cells[3,5]='') then
    begin
      StringGrid1.Cells[3,1]:='идет...' ;
      panel8.Color:=clRed;
      panel8.Caption:='Раунд 1';
    end;

// определяем конец первого раунда и начинаем первый перерыв

  if (Labelm.Caption=StringGrid1.Cells[1,1]) and (Labels.Caption=stringgrid1.Cells[2,1]) and (StringGrid1.Cells[3,1]='идет...') then
    begin
      StringGrid1.Cells[3,1]:='Окончен' ;     // признак окончания первого раунда
      StringGrid1.Cells[3,2]:='идет...' ;     // признак работы первого перерыва
      labelm.Caption:='0';       // сброс счетчика
      labels.Caption:='0';
      labelms.Caption:='0';
      panel8.Color:=clLime;
      panel8.Caption:='';

      //переносим результат первого раунда в панель результата первого раунда
      ...
      //уносим итог раунда в общий итог
...
// определяем конец первого перерыва и начинаем второй раунд

  if (Labelm.Caption=StringGrid1.Cells[1,2]) and (Labels.Caption=stringgrid1.Cells[2,2]) and (StringGrid1.Cells[3,2]='идет...') then
    begin
      StringGrid1.Cells[3,2]:='Окончен' ;
      StringGrid1.Cells[3,3]:='идет...' ;
      labelm.Caption:='0';       // сброс счетчика
      labels.Caption:='0';
      labelms.Caption:='0';
      panel8.Color:=clRed;
...
// определяем конец второго раунда и начинаем второй перерыв

  if (Labelm.Caption=StringGrid1.Cells[1,3]) and (Labels.Caption=stringgrid1.Cells[2,3]) and (StringGrid1.Cells[3,3]='идет...') then
    begin
      StringGrid1.Cells[3,3]:='Окончен' ;
      StringGrid1.Cells[3,4]:='идет...' ;
      labelm.Caption:='0';       // сброс счетчика
      labels.Caption:='0';
      labelms.Caption:='0';
      panel8.Color:=clLime;
...
// определяем конец второго перерыва и начинаем третий раунд

  if (Labelm.Caption=StringGrid1.Cells[1,4]) and (Labels.Caption=stringgrid1.Cells[2,4]) and (StringGrid1.Cells[3,4]='идет...') then
    begin
      StringGrid1.Cells[3,4]:='Окончен' ;
      StringGrid1.Cells[3,5]:='идет...' ;
      labelm.Caption:='0';       // сброс счетчика
      labels.Caption:='0';
      labelms.Caption:='0';
      panel8.Color:=clRed;
...
// определяем конец третьего раунда и заканчиваем бой

  if (Labelm.Caption=StringGrid1.Cells[1,5]) and (Labels.Caption=stringgrid1.Cells[2,5]) and (StringGrid1.Cells[3,5]='идет...') then
    begin
      StringGrid1.Cells[3,5]:='Окончен' ;
 //     StringGrid1.Cells[3,5]:='идет...' ;
      labelm.Caption:='0';       // сброс счетчика
      labels.Caption:='0';
      labelms.Caption:='0';
